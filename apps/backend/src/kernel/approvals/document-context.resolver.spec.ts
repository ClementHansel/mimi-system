...
import { ApprovalDocumentType, isRoleAuthorized, LocationType, ReturnDirection, RoleKey } from '@mimi/shared';
...
describe('resolveDocumentContext', () => {
  it('resolves stock_opname at an outlet to variant "outlet"', async () => {
    ...
    const ctx = await resolveDocumentContext(client as never, ApprovalDocumentType.STOCK_OPNAME, 'doc-1');
  ...
  it('resolves waste at a warehouse to variant "warehouse"', async () => {
    const client = makeClient({ 'FROM waste_records': [{ location_type: LocationType.WAREHOUSE }] });
  ...
  it('resolves return direction verbatim from the returns table', async () => {
    const client = makeClient({ 'FROM returns': [{ direction: ReturnDirection.WAREHOUSE_TO_SUPPLIER }] });
  ...
  it('returns an empty context (no query) for document types with no irregular chain', async () => {
    ...
    const ctx = await resolveDocumentContext(client as never, ApprovalDocumentType.PURCHASE_ORDER, 'doc-1');
    expect(ctx).toEqual({});
  ...
  it('resolves to an undefined variant when the joined row is missing (document not found)', async () => {
    const client = makeClient({});
    const ctx = await resolveDocumentContext(client as never, ApprovalDocumentType.STOCK_OPNAME, 'ghost');
...
describe('resolveDocumentContextsBatch', () => {
  it('batches stock_opname location types for many ids in one query', async () => {
    ...
    const result = await resolveDocumentContextsBatch(client as never, ApprovalDocumentType.STOCK_OPNAME, ['a', 'b']);
  ...
  it('returns an empty map without querying when there are no ids', async () => {
    const client = makeClient({});
    const result = await resolveDocumentContextsBatch(client as never, ApprovalDocumentType.STOCK_OPNAME, []);
  ...
  it('maps every id to {} for regular document types without querying', async () => {
    const client = makeClient({});
    const result = await resolveDocumentContextsBatch(client as never, ApprovalDocumentType.PAYROLL_RUN, ['x', 'y']);
...
describe('resolveEligibleRoles — carried-forward item 2', () => {
  it('stock_opname step 1 narrows to Supervisor at an outlet', () => {
    const roles = resolveEligibleRoles(ApprovalDocumentType.STOCK_OPNAME, 1, RoleKey.SUPERVISOR, { variant: 'outlet' });
  ...
  it('stock_opname step 1 narrows to Kepala Gudang at a warehouse — the seeded row is only "supervisor"', () => {
    const roles = resolveEligibleRoles(ApprovalDocumentType.STOCK_OPNAME, 1, RoleKey.SUPERVISOR, { variant: 'warehouse' });
  ...
  it('stock_opname step 2 (manager threshold) is unaffected by the override — passthrough', () => {
    const roles = resolveEligibleRoles(ApprovalDocumentType.STOCK_OPNAME, 2, RoleKey.MANAGER, { variant: 'warehouse' });
    expect(roles).toEqual([RoleKey.MANAGER]);
  });

  it('waste step 1 narrows by location type identically to opname', () => {
    expect(resolveEligibleRoles(ApprovalDocumentType.WASTE, 1, RoleKey.SUPERVISOR, { variant: 'outlet' })).toEqual([RoleKey.SUPERVISOR]);
    expect(resolveEligibleRoles(ApprovalDocumentType.WASTE, 1, RoleKey.SUPERVISOR, { variant: 'warehouse' })).toEqual([RoleKey.KEPALA_GUDANG]);
  });

  it('return step 1 narrows by direction — outlet_to_warehouse leg goes to Supervisor', () => {
    const roles = resolveEligibleRoles(ApprovalDocumentType.RETURN, 1, RoleKey.SUPERVISOR, { variant: ReturnDirection.OUTLET_TO_WAREHOUSE });
    expect(roles).toEqual([RoleKey.SUPERVISOR]);
  });

  it('return step 1 narrows by direction — warehouse_to_supplier leg goes to Kepala Gudang', () => {
    const roles = resolveEligibleRoles(ApprovalDocumentType.RETURN, 1, RoleKey.SUPERVISOR, { variant: ReturnDirection.WAREHOUSE_TO_SUPPLIER });
    expect(roles).toEqual([RoleKey.KEPALA_GUDANG]);
  });

  it('leave_request step 1 broadens to the any-of set regardless of context', () => {
    const roles = resolveEligibleRoles(ApprovalDocumentType.LEAVE_REQUEST, 1, RoleKey.SUPERVISOR, {});
...
describe('property: no runtime-resolved role escapes the expected set', () => {
  const scenarios: Array<{
    ...
    { label: 'opname @ outlet', documentType: ApprovalDocumentType.STOCK_OPNAME, context: { variant: 'outlet' }, expectedEligible: [RoleKey.SUPERVISOR] },
    { label: 'opname @ warehouse', documentType: ApprovalDocumentType.STOCK_OPNAME, context: { variant: 'warehouse' }, expectedEligible: [RoleKey.KEPALA_GUDANG] },
    { label: 'waste @ outlet', documentType: ApprovalDocumentType.WASTE, context: { variant: 'outlet' }, expectedEligible: [RoleKey.SUPERVISOR] },
    { label: 'waste @ warehouse', documentType: ApprovalDocumentType.WASTE, context: { variant: 'warehouse' }, expectedEligible: [RoleKey.KEPALA_GUDANG] },
    { label: 'return outlet_to_warehouse', documentType: ApprovalDocumentType.RETURN, context: { variant: ReturnDirection.OUTLET_TO_WAREHOUSE }, expectedEligible: [RoleKey.SUPERVISOR] },
    { label: 'return warehouse_to_supplier', documentType: ApprovalDocumentType.RETURN, context: { variant: ReturnDirection.WAREHOUSE_TO_SUPPLIER }, expectedEligible: [RoleKey.KEPALA_GUDANG] },
    { label: 'leave_request (any-of)', documentType: ApprovalDocumentType.LEAVE_REQUEST, context: {}, expectedEligible: [RoleKey.SUPERVISOR, RoleKey.HR_ADMIN] },
  ];
  ...
  for (const scenario of scenarios) {
    it(`${scenario.label}: every role's eligibility matches the CONTRACTS-transcribed expectation, for any stored seed role`, () => {
      ...
      const expectedWithOverride = new Set<RoleKey>([...scenario.expectedEligible, RoleKey.OWNER, RoleKey.MANAGER]);

      fc.assert(
        fc.property(fc.constantFrom(...ALL_ROLES), fc.constantFrom(...ALL_ROLES), (actorRole, seededRole) => {
          const eligible = resolveEligibleRoles(scenario.documentType, 1, seededRole, scenario.context);
