...
const DECISION_OPS: Record<string, DecisionOpSet> = {
  void_refunds: { offlineOp: 'approved_offline', onlineApprove: 'approved', onlineReject: 'rejected' },
  replenishment_requests: { offlineOp: 'supervisor_approved_offline', onlineApprove: 'supervisor_approved', onlineReject: 'supervisor_rejected' },
  waste_records: { offlineOp: 'approved_offline', onlineApprove: 'approved', onlineReject: 'rejected' },
};
...
export class ConflictDetectorService {
  ...
  async recordPoison(client: DbClient, event: Pick<SyncEventEnvelope, 'eventId' | 'entity' | 'entityId' | 'locationId' | 'op'>, code: string, detail: string): Promise<void> {
   ...
   */
  async recordProjectionFailure(client: DbClient, event: SyncEventEnvelope, error: string): Promise<void> {
    await this.conflicts.recordConflictIfAbsent(client, {
      kind: 'poison',
      queue: 'exception',
      entity: event.entity,
   ...
   */
  async recordSeqConflict(client: DbClient, incoming: SyncEventEnvelope, existingEventId: UUID | undefined, locationId: UUID | null): Promise<void> {
    await this.conflicts.recordConflictIfAbsent(client, {
      kind: 'poison',
      queue: 'conflict',
  ...
  async detectAtApply(client: DbClient, event: SyncEventEnvelope): Promise<{ isLoser: boolean }> {
    ...
    switch (event.entity) {
      ...
      case 'attendance':
        ...
        break;
      case 'online_orders':
        if (event.op === 'recorded') isLoser = await this.detectDuplicatePlatformOrder(client, event);
        break;
      default:
        break;
    }
    if (DECISION_OPS[event.entity]) {
      isLoser = (await this.detectDecisionRace(client, event, DECISION_OPS[event.entity]!)) || isLoser;
    }
    return { isLoser };
  }
  ...
  private async detectDoubleCount(client: DbClient, event: SyncEventEnvelope): Promise<void> {
    ...
    const siblings = await this.events.findByEntityId(client, 'stock_opname', event.entityId, event.eventId);
    for (const sib of siblings) {
      ...
      const sharedItemIds = mine.lines.map((l) => l.itemId).filter((id) => theirs.lines.some((t) => t.itemId === id));
  ...
  private async detectDuplicateReceipt(client: DbClient, event: SyncEventEnvelope): Promise<boolean> {
    const siblings = (await this.events.findByEntityId(client, 'sj_drops', event.entityId, event.eventId)).filter(
      (s) => s.op === 'received',
    );
    ...
    await this.conflicts.recordConflictIfAbsent(client, {
      ...
      loserEventId: event.eventId,
      detail: { note: 'second sj_drops.received for the same drop; stock effect posts once (winner only)' },
  ...
  private async detectDecisionRace(client: DbClient, event: SyncEventEnvelope, ops: DecisionOpSet): Promise<boolean> {
    const isDecision = event.op === ops.offlineOp || event.op === ops.onlineApprove || event.op === ops.onlineReject;
    if (!isDecision) return false;

    const siblings = (await this.events.findByEntityId(client, event.entity, event.entityId, event.eventId)).filter(
    ...
    for (const sib of siblings) {
      ...
      const loserWasOfflineApproval = (loserEventId === sib.event_id ? sib.op : event.op) === ops.offlineOp;
      await this.conflicts.recordConflictIfAbsent(client, {
        kind: 'decision_race',
        ...
        detail: { winnerOp: winnerEventId === event.eventId ? event.op : sib.op, loserOp: loserEventId === event.eventId ? event.op : sib.op },
   ...
   */
  private async detectDuplicatePlatformOrder(client: DbClient, event: SyncEventEnvelope): Promise<boolean> {
