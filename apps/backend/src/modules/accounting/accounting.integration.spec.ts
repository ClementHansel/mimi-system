...
import { AccountType, ERR_PROOF_REQUIRED, ERR_UNBALANCED_ENTRY, PayeeType, PaymentVerificationRefType, RoleKey } from '@mimi/shared';
...
import { appPoolForDi, asCommittedRequest, asRequest, closePool, loadFixtures, withRollbackAs, type Fixtures } from './test-support/live-db';
...
describe('M17 accounting — live DB integration', () => {
  ...
  it('POST periods/:id/close and /reopen also return camelCase, and both persist past their own request', async () => {
    const owner = { role: RoleKey.OWNER, userId: fixtures.usersByRole[RoleKey.OWNER], locationIds: [] };
    ...
    try {
      const closed = await asRequest(owner, (client) => periods.close(client, openPeriod!.id, fixtures.usersByRole[RoleKey.OWNER], undefined));
      ...
      const reopened = await asRequest(owner, (client) => periods.reopen(client, openPeriod!.id, 'test: verifying camelCase shape'));
      expect(reopened.status).toBe('open');
      expect(reopened).not.toHaveProperty('closed_at');

      const rereadReopened = await asRequest(owner, (client) => periods.get(client, openPeriod!.id));
      ...
      if (finalState.status === 'closed') {
        await asRequest(owner, (client) => periods.reopen(client, openPeriod!.id, 'test cleanup: restore to open'));
      }
    }
  });
  ...
  it('a balanced manual entry posts and persists past its own request', async () => {
    const finance = { role: RoleKey.FINANCE, userId: fixtures.usersByRole[RoleKey.FINANCE], locationIds: [] };
  ...
  it('write-then-read-back: reverse() posts a swapped-legs entry and flips the original to reversed, both surviving past their own requests', async () => {
    ...
    const reversal = await asRequest(finance, (client) => journal.reverse(client, fixtures.usersByRole[RoleKey.FINANCE], original.id, 'test: reversing entry'));
    ...
    const rereadOriginal = await asRequest(finance, (client) => journal.getDetail(client, original.id));
    expect(rereadOriginal.status).toBe('reversed');
    const rereadReversal = await asRequest(finance, (client) => journal.getDetail(client, reversal.id));
  ...
  it('CARRIED ITEM #3 FIX: PaymentVerificationsService.create() succeeds for a real kasir session (escalated insert) and the row persists for a later central-role read', async () => {
    const kasir = { role: RoleKey.KASIR, userId: fixtures.usersByRole[RoleKey.KASIR], locationIds: [fixtures.outletId] };
    ...
    const reread = await asRequest({ role: RoleKey.FINANCE, userId: fixtures.usersByRole[RoleKey.FINANCE], locationIds: [] }, (client) => payments.getDetail(client, created.id));
  ...
  it('Pending -> Verified -> Paid ladder: verify without proof is ERR_PROOF_REQUIRED; the happy path completes end to end across separate requests, dispatching the right journal.action', async () => {
    ...
    await asRequest(finance, (client) =>
      expect(payments.verify(client, financeActor, pv.id, undefined)).rejects.toMatchObject({ response: { code: ERR_PROOF_REQUIRED } }),
    );
    ...
    await asRequest(finance, (client) => payments.uploadProof(client, financeActor, pv.id, attachmentId));

    const verified = await asRequest(finance, (client) => payments.verify(client, financeActor, pv.id, 'ok'));
    ...
    try {
      const paid = await asRequest(finance, (client) => payments.pay(client, financeActor, pv.id, { paidVia: 'cash' }));
  ...
  it('write-then-read-back: reject() persists past its own request and never transitions to paid', async () => {
    ...
    const rejected = await asRequest(finance, (client) => payments.reject(client, financeActor, pv.id, 'test: rejecting this PV'));
  ...
  describe('write-then-read-back across SEPARATE connections (each simulating one real HTTP request)', () => {
    it('chart-of-accounts: create() persists past its own request, and update() persists past ITS own separate request', async () => {
      ...
      try {
        const created = await asRequest(finance, (client) =>
          coa.create(client, { code, name: 'BE-TXN-ROLLBACK test account', type: AccountType.EXPENSE, normalBalance: 'debit', isPostable: true }),
        );
        ...
        const updated = await asRequest(finance, (client) => coa.update(client, created.id, { name: 'BE-TXN-ROLLBACK renamed' }));
        ...
        await asCommittedRequest({ role: RoleKey.OWNER, userId: fixtures.usersByRole[RoleKey.OWNER], locationIds: [] }, (client) =>
          client.query(`DELETE FROM chart_of_accounts WHERE code = $1`, [code]),
        );
      }
    });
    ...
    it('exceptions: recordVerdict() persists past its own request (D-17 finance exception queue)', async () => {
      ...
      const { caseId, oaId } = await asCommittedRequest({ role: RoleKey.OWNER, userId: approverId, locationIds: [] }, async (client) => {
      ...
      const verdict = await asRequest(owner, (client) => exceptions.recordVerdict(client, fixtures.usersByRole[RoleKey.OWNER], caseId, { verdict: 'rejected', reason: 'test: rejecting this case' }));
      ...
      const reread = await asRequest(owner, async (client) => {
        const oa = await client.query<{ verdict: string | null; reviewed_by: string | null }>(`SELECT verdict, reviewed_by FROM offline_authorizations WHERE id = $1`, [oaId]);
        const sc = await client.query<{ status: string; resolution: string | null }>(`SELECT status, resolution FROM sync_conflicts WHERE id = $1`, [caseId]);
