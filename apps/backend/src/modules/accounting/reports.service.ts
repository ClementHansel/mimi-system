...
interface TrialBalanceLine { accountCode: string; accountName: string; type: string; debit: Money; credit: Money }
interface PLLine { accountCode: string; name: string; amount: Money }
...
export class ReportsService {
  async trialBalance(client: PoolClient, periodCode: string): Promise<{ rows: TrialBalanceLine[]; totalDebit: Money; totalCredit: Money; balanced: boolean }> {
    const period = await client.query<{ id: string }>(`SELECT id FROM fiscal_periods WHERE period_code = $1`, [periodCode]);
    if (!period.rows[0]) throw new NotFoundException({ code: ERR_NOT_FOUND, message: `Fiscal period '${periodCode}' not found` });

    const res = await client.query<{ code: string; name: string; type: string; debit: Money; credit: Money }>(
    ...
    const rows: TrialBalanceLine[] = res.rows.map((r) => {
      ...
      return { accountCode: r.code, accountName: r.name, type: r.type, debit: r.debit, credit: r.credit };
    });

    return { rows, totalDebit, totalCredit, balanced: totalDebit === totalCredit };
  }

  async profitLoss(client: PoolClient, from: string, to: string, locationId?: string): Promise<{ revenue: PLLine[]; expenses: PLLine[]; totalRevenue: Money; totalExpense: Money; netProfit: Money }> {
    const args: unknown[] = [from, to];
    let locFilter = '';
    if (locationId) { args.push(locationId); locFilter = `AND je.location_id = $3`; }
    ...
    for (const r of rows.rows) {
      if (r.type === 'revenue') { revenue.push({ accountCode: r.code, name: r.name, amount: r.amount }); totalRevenue = addMoney(totalRevenue, r.amount); }
      else { expenses.push({ accountCode: r.code, name: r.name, amount: r.amount }); totalExpense = addMoney(totalExpense, r.amount); }
    }
    return { revenue, expenses, totalRevenue, totalExpense, netProfit: subMoney(totalRevenue, totalExpense) };
  }

  async balanceSheet(client: PoolClient, asOf: string): Promise<{ assets: PLLine[]; liabilities: PLLine[]; equity: PLLine[]; balanced: boolean }> {
    ...
    for (const r of res.rows) {
      ...
      if (r.type === 'asset') { assets.push(line); totalAssets = addMoney(totalAssets, r.balance); }
      else if (r.type === 'liability') { liabilities.push(line); totalLiabEquity = addMoney(totalLiabEquity, r.balance); }
      else { equity.push(line); totalLiabEquity = addMoney(totalLiabEquity, r.balance); }
    }
  ...
  async stockValue(client: PoolClient, locationId?: string): Promise<{ locationId: string; locationName: string; value: Money; byCategory: { categoryName: string; value: Money }[] }[]> {
    const args: unknown[] = [];
    let where = '';
    if (locationId) { args.push(locationId); where = 'WHERE sb.location_id = $1'; }

    const res = await client.query<{ location_id: string; location_name: string; category_name: string; value: Money }>(
    ...
    const byLocation = new Map<string, { locationId: string; locationName: string; value: Money; byCategory: { categoryName: string; value: Money }[] }>();
    for (const r of res.rows) {
      let entry = byLocation.get(r.location_id);
      if (!entry) {
        entry = { locationId: r.location_id, locationName: r.location_name, value: ZERO_MONEY, byCategory: [] };
