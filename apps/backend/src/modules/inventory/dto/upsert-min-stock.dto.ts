...
export class MinStockRuleUpsertLine {
  ...
  @Matches(QTY_PATTERN, {
    message: 'minQty must be a decimal string with up to 3 fractional digits, e.g. "12.500"',
  })
  minQty!: string;

  @IsOptional()
  @Matches(QTY_PATTERN, {
    message: 'reorderQty must be a decimal string with up to 3 fractional digits, e.g. "20.000"',
  })
  reorderQty?: string;
}
