...
export class PeriodsService {
  async list(client: PoolClient, page = 1, pageSize = 50): Promise<Paginated<PeriodRow>> {
    const countRes = await client.query<{ count: string }>('SELECT COUNT(*) AS count FROM payroll_periods');
  ...
  async create(client: PoolClient, periodCode: string): Promise<PeriodRow> {
    ...
    const existing = await client.query<{ id: UUID }>('SELECT id FROM payroll_periods WHERE period_code = $1', [periodCode]);
  ...
  async requirePeriod(client: PoolClient, id: UUID): Promise<{ id: UUID; periodCode: string; startDate: string; endDate: string; status: string }> {
    const res = await client.query<Record<string, any>>('SELECT * FROM payroll_periods WHERE id = $1', [id]);
    if (res.rows.length === 0) throw new NotFoundException({ code: ERR_NOT_FOUND, message: 'Payroll period not found' });
    const r = res.rows[0]!;
    return { id: r.id, periodCode: r.period_code, startDate: pgDateToIso(r.start_date), endDate: pgDateToIso(r.end_date), status: r.status };
  }

  async markStatus(client: PoolClient, id: UUID, status: 'open' | 'processing' | 'closed'): Promise<void> {
    await client.query('UPDATE payroll_periods SET status = $2 WHERE id = $1', [id, status]);
  }

  private mapPeriod(r: Record<string, any>, runs: { id: UUID; run_number: string; status: string }[]): PeriodRow {
    return {
      id: r.id,
