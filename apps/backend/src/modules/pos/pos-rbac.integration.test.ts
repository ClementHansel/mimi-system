...
import { ERR_APPROVAL_STEP_ROLE, ERR_AUTH_PIN_INVALID, ERR_REASON_REQUIRED, PaymentMethod, RoleKey, VoidRefundType } from '@mimi/shared';
...
function services(pool = getAppPool()) {
  ...
  return {
    ...
    voidRefunds: new PosVoidRefundService(pool, approvals, stockLedger, syncEmit, notifications, eventBus),
...
async function openShiftWithCashSale(client: PoolClient, svc: ReturnType<typeof services>) {
  ...
  const shift = await svc.shifts.open(client, fx.kasirId, { clientId: randomUUID(), locationId: fx.locationId, openingCash: '50000.00' });
...
describe('POS RBAC — negative and positive paths, live database', () => {
  it('a Kasir with NO pin_hash configured is rejected at the PIN gate before the role gate is ever reached', async () => {
    await withRollback({ userId: fx.kasirId, roleKey: 'kasir', locationIds: [fx.locationId] }, async (client) => {
      const svc = services();
      ...
      await expect(svc.voidRefunds.approve(client, requested.voidRefundId, fx.kasirId, RoleKey.KASIR, '000000')).rejects.toMatchObject({
  ...
  it('a Kasir CANNOT approve their own void even WITH a correct PIN — ApprovalService rejects the role, not just the credential', async () => {
    ...
    const original = await ownerPool.query<{ pin_hash: string | null }>('SELECT pin_hash FROM users WHERE id = $1', [fx.kasirId]);
    ...
    try {
      await withRollback({ userId: fx.kasirId, roleKey: 'kasir', locationIds: [fx.locationId] }, async (client) => {
        ...
        await expect(svc.voidRefunds.approve(client, requested.voidRefundId, fx.kasirId, RoleKey.KASIR, KASIR_TEST_PIN)).rejects.toMatchObject({
          response: { code: ERR_APPROVAL_STEP_ROLE },
        });

        // And the void is genuinely still pending — the rejected attempt had NO side effect.
        const stillPending = await client.query('SELECT status FROM void_refunds WHERE id = $1', [requested.voidRefundId]);
        expect(stillPending.rows[0].status).toBe('pending');
      });
    } finally {
      await ownerPool.query('UPDATE users SET pin_hash = $2 WHERE id = $1', [fx.kasirId, original.rows[0]?.pin_hash ?? null]);
    }
  }, 30_000);
  ...
  it('a Supervisor (the eligible role) CAN approve the same void — proves the gate is role-specific, not a blanket block', async () => {
    await withRollback({ userId: fx.kasirId, roleKey: 'kasir', locationIds: [fx.locationId] }, async (client) => {
      ...
      await switchActor(client, { userId: fx.supervisorId, roleKey: 'supervisor', locationIds: [fx.locationId] });
      const approved = await svc.voidRefunds.approve(client, requested.voidRefundId, fx.supervisorId, RoleKey.SUPERVISOR, fx.supervisorPin);
  ...
  it('a Kasir CANNOT decide a cash-variance proposal (D-19 — online-only, supervisor+ only)', async () => {
    await withRollback({ userId: fx.kasirId, roleKey: 'kasir', locationIds: [fx.locationId] }, async (client) => {
      ...
      await neutralizeOpenShifts(client, fx.locationId);
      const shift = await svc.shifts.open(client, fx.kasirId, { clientId: randomUUID(), locationId: fx.locationId, openingCash: '50000.00' });
      const { report } = await svc.shifts.close(client, shift.id, fx.kasirId, { closingCashCounted: '0.00' });
      expect(report.cashVarianceProposalId).not.toBeNull();

      await expect(
        svc.cashVariances.approve(client, report.cashVarianceProposalId!, fx.kasirId, RoleKey.KASIR, 'kasir mencoba menyetujui usulan sendiri'),
  ...
  it('a Supervisor CAN decide a cash-variance proposal, but a reason is mandatory even to approve (§5.9)', async () => {
    await withRollback({ userId: fx.kasirId, roleKey: 'kasir', locationIds: [fx.locationId] }, async (client) => {
      ...
      await expect(svc.cashVariances.approve(client, proposalId, fx.supervisorId, RoleKey.SUPERVISOR, '')).rejects.toMatchObject({
        response: { code: ERR_REASON_REQUIRED },
      });

      const decided = await svc.cashVariances.approve(client, proposalId, fx.supervisorId, RoleKey.SUPERVISOR, 'Disetujui setelah konfirmasi kasir');
