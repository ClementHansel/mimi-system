...
describe('POS — full shift, live database', () => {
  ...
  it('open -> sell cash/QRIS/transfer -> void the cash sale -> online order -> close with a cash-variance proposal', async () => {
    await withRollback({ userId: fx.kasirId, roleKey: 'kasir', locationIds: [fx.locationId] }, async (client) => {
      ...
      const reopened = await svc.shifts.open(client, fx.kasirId, { clientId: openClientId, locationId: fx.locationId, openingCash });
      ...
      await switchActor(client, { userId: fx.kasirId, roleKey: 'kasir', locationIds: [fx.locationId] });
      ...
      const movementCount = await client.query(`SELECT COUNT(*) FROM stock_movements WHERE ref_type = 'sale' AND ref_id = $1`, [cashSale.id]);
      ...
      const reversalCount = await client.query(`SELECT COUNT(*) FROM stock_movements WHERE ref_type = 'void_refund' AND movement_type = 'return_in'`, []);
      ...
      const { shift: closedShift, report } = await svc.shifts.close(client, shift.id, fx.kasirId, { closingCashCounted });
      ...
      expect(report.onlineOrders.find((o) => o.platform === OnlinePlatform.GOFOOD)?.net).toBe(netReceived);

      const proposalId = report.cashVarianceProposalId!;
      const proposalRow = await client.query(`SELECT status, amount FROM cash_variance_proposals WHERE id = $1`, [proposalId]);
      ...
      const decided = await svc.cashVariances.approve(client, proposalId, fx.supervisorId, RoleKey.SUPERVISOR, 'Disetujui, potong gaji bulan ini');
    ...
    await withRollback({ userId: fx.kasirId, roleKey: 'kasir', locationIds: [fx.locationId] }, async (client) => {
      ...
      const sale = await svc.sales.create(
        ...
        fx.kasirId,
        {
          ...
          payments: [{ method: PaymentMethod.CASH, amount: (Number(fx.productPrice) * 2).toFixed(2) }],
        },
        { roleKey: 'kasir', locationIds: [fx.locationId] },
      );
  ...
  it('a BATCH recipe (yield_qty > 1) posts stock scaled by qtySold ÷ yieldQty — regression guard for the flat-multiply bug', async () => {
    await withRollback({ userId: fx.kasirId, roleKey: 'kasir', locationIds: [fx.locationId] }, async (client) => {
      ...
      await client.query(`INSERT INTO recipe_lines (recipe_id, item_id, qty, unit_id) VALUES ($1,$2,'2.000',$3)`, [
        testRecipeId,
        testItemId,
        unitId,
      ]);
