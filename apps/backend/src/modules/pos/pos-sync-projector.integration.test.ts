...
function services(pool: Pool) {
  ...
  const voidRefunds = new PosVoidRefundService(pool, approvals, stockLedger, syncEmit, notifications, eventBus);
...
function mkEvent(params: {
  ...
  return {
    ...
    op: params.op,
    payload: { v: 1, data: params.data, meta: { actorUserId: params.actorUserId, actorRole: 'kasir', appVersion: '1.0.0' } },
...
async function cleanupRealIngestArtifacts(ownerPool: Pool, params: { originDeviceId: string; saleId: string; shiftId: string }): Promise<void> {
  const keys = await ownerPool.query<{ location_id: string; storage_area_id: string; item_id: string }>(
    `SELECT DISTINCT location_id, storage_area_id, item_id FROM stock_movements WHERE ref_type = 'sale' AND ref_id = $1`,
    [params.saleId],
  );
  await ownerPool.query(`DELETE FROM stock_movements WHERE ref_type = 'sale' AND ref_id = $1`, [params.saleId]);
  ...
  await ownerPool.query(`DELETE FROM sync_events WHERE origin_device_id = $1`, [params.originDeviceId]);
  await ownerPool.query(`DELETE FROM sync_batches WHERE origin_device_id = $1`, [params.originDeviceId]);
  await ownerPool.query(`DELETE FROM sync_cursors WHERE subscriber_id = $1`, [params.originDeviceId]);
}
...
describe('PosSyncProjector — the domain-projection hook, real ingest, live database', () => {
  it('a real offline-shaped shift + sale (bank_transfer) syncs through real SyncIngestService.ingestBatch into real sales/sale_lines/sale_payments rows with the correct payment status, posts stock, and is idempotent on replay', async () => {
    ...
    const resolveLocation = async (id: string) => (id === originDeviceId ? fx.locationId : undefined);
    ...
    try {
      const shiftOpenedEvent = mkEvent({
        ...
        data: { clientId: shiftClientId, locationId: fx.locationId, openingCash: '100000.00', openedAt: new Date().toISOString(), shiftNumber: `${fx.locationCode}-SIM1-S1` },
      });
      ...
      const shiftRow = await ownerPool.query(`SELECT id, status, shift_number FROM pos_shifts WHERE id = $1`, [shiftId]);
      expect(shiftRow.rows[0]?.status).toBe('open');
      expect(shiftRow.rows[0]?.shift_number).toBe(`${fx.locationCode}-SIM1-S1`);

      const saleRow = await ownerPool.query(`SELECT id, status, receipt_number, client_id FROM sales WHERE id = $1`, [saleId]);
      ...
      const lineRows = await ownerPool.query(`SELECT product_id FROM sale_lines WHERE sale_id = $1`, [saleId]);
      expect(lineRows.rows).toHaveLength(1);

      const paymentRows = await ownerPool.query(`SELECT method, payment_status FROM sale_payments WHERE sale_id = $1`, [saleId]);
      ...
      const replayAck = await ingest.ingestBatch(batchOf([shiftOpenedEvent, saleEvent]), resolveLocation);
      expect(replayAck.rejected).toEqual([]);

      const saleCountAfterReplay = await ownerPool.query(`SELECT COUNT(*) FROM sales WHERE id = $1`, [saleId]);
      expect(Number(saleCountAfterReplay.rows[0]!.count)).toBe(1);
      const lineCountAfterReplay = await ownerPool.query(`SELECT COUNT(*) FROM sale_lines WHERE sale_id = $1`, [saleId]);
      expect(Number(lineCountAfterReplay.rows[0]!.count)).toBe(1);
      const paymentCountAfterReplay = await ownerPool.query(`SELECT COUNT(*) FROM sale_payments WHERE sale_id = $1`, [saleId]);
...
describe('PosSyncProjector — direct-call guarantees (rolled back, no cleanup needed)', () => {
  it('is idempotent even when called TWICE for the same event, bypassing SyncProjectorRegistry\'s own dedupe entirely', async () => {
    await withRollback({ userId: fx.kasirId, roleKey: 'owner', locationIds: [] }, async (client) => {
      ...
      const shiftEvent = mkEvent({
        ...
        op: 'opened',
        actorUserId: fx.kasirId,
        data: { clientId: randomUUID(), locationId: fx.locationId, openingCash: '50000.00', openedAt: new Date().toISOString(), shiftNumber: `${fx.locationCode}-SIM2-S1` },
      });
      ...
      const lineCount = await client.query(`SELECT COUNT(*) FROM sale_lines WHERE sale_id = $1`, [saleId]);
      expect(Number(lineCount.rows[0].count)).toBe(1);
      const paymentCount = await client.query(`SELECT COUNT(*) FROM sale_payments WHERE sale_id = $1`, [saleId]);
      expect(Number(paymentCount.rows[0].count)).toBe(1);
      const movementCount = await client.query(`SELECT COUNT(*) FROM stock_movements WHERE ref_type = 'sale' AND ref_id = $1`, [saleId]);
      ...
      await svc.projector.project(client, saleEvent, { isConflictLoser: false });
      const movementCountAgain = await client.query(`SELECT COUNT(*) FROM stock_movements WHERE ref_type = 'sale' AND ref_id = $1`, [saleId]);
    ...
    await withRollback({ userId: fx.kasirId, roleKey: 'owner', locationIds: [] }, async (client) => {
      ...
      await svc.projector.project(
        ...
        mkEvent({
          ...
          op: 'opened',
          actorUserId: fx.kasirId,
          data: { clientId: randomUUID(), locationId: fx.locationId, openingCash: '50000.00', openedAt: new Date().toISOString(), shiftNumber: `${fx.locationCode}-SIM3-S1` },
        }),
  ...
  it('isConflictLoser=true on void_refunds.approved_offline is NOT applied — an online decision already won the C3 race', async () => {
    await withRollback({ userId: fx.kasirId, roleKey: 'owner', locationIds: [] }, async (client) => {
      ...
      await svc.projector.project(
        client,
        mkEvent({
          originDeviceId, clientSeq: 1, locationId: fx.locationId, entity: 'pos_shifts', entityId: shiftId, op: 'opened', actorUserId: fx.kasirId,
          data: { clientId: randomUUID(), locationId: fx.locationId, openingCash: '50000.00', openedAt: new Date().toISOString(), shiftNumber: `${fx.locationCode}-SIM4-S1` },
        }),
        { isConflictLoser: false },
      );
      await svc.projector.project(
        client,
        mkEvent({
          originDeviceId, clientSeq: 2, locationId: fx.locationId, entity: 'sales', entityId: saleId, op: 'completed', actorUserId: fx.kasirId,
          data: {
            clientId: randomUUID(), locationId: fx.locationId, shiftId, occurredAt: new Date().toISOString(),
            lines: [{ productId: fx.productId, qty: '1.000', unitPrice: fx.productPrice }],
            payments: [{ method: PaymentMethod.CASH, amount: fx.productPrice }],
      ...
      await svc.projector.project(
        client,
        mkEvent({
          originDeviceId, clientSeq: 3, locationId: fx.locationId, entity: 'void_refunds', entityId: saleId, op: 'requested', actorUserId: fx.kasirId,
          data: { clientId: randomUUID(), type: VoidRefundType.VOID, reason: 'test — C3 loser path' },
        }),
        { isConflictLoser: false },
      );
      ...
      await svc.projector.project(
        client,
        mkEvent({
          originDeviceId, clientSeq: 4, locationId: fx.locationId, entity: 'void_refunds', entityId: saleId, op: 'approved_offline', actorUserId: fx.kasirId,
          data: {},
        }),
        { isConflictLoser: true },
      );

      const voidRow = await client.query(`SELECT status, offline_authorized FROM void_refunds WHERE sale_id = $1`, [saleId]);
  ...
  it('isConflictLoser=true on online_orders.recorded (C8) never creates a second row — CONTRACTS\' UNIQUE(platform, order_ref) makes that structurally impossible; the winner row is returned unchanged', async () => {
    await withRollback({ userId: fx.kasirId, roleKey: 'owner', locationIds: [] }, async (client) => {
      ...
      const orderData = {
        clientId: randomUUID(), locationId: fx.locationId, platform: OnlinePlatform.GOFOOD, orderRef,
        orderDate: new Date().toISOString().slice(0, 10), grossAmount: '50000.00', discountAmount: '0.00',
        platformFee: '0.00', otherFee: '0.00', netReceived: '50000.00', status: OnlineOrderStatus.COMPLETED,
      };
      const firstId = randomUUID();
      const first = mkEvent({ originDeviceId: randomUUID(), clientSeq: 1, locationId: fx.locationId, entity: 'online_orders', entityId: firstId, op: 'recorded', actorUserId: fx.kasirId, data: orderData });
      const second = mkEvent({ originDeviceId: randomUUID(), clientSeq: 1, locationId: fx.locationId, entity: 'online_orders', entityId: randomUUID(), op: 'recorded', actorUserId: fx.kasirId, data: { ...orderData, clientId: randomUUID() } });
