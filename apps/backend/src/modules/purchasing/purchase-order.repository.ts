...
export class PurchaseOrderRepository {
  ...
  async setTotals(
    ...
    await client.query(
      `UPDATE purchase_orders SET subtotal = $2, tax = $3, total = $4, updated_at = NOW() WHERE id = $1`,
      [poId, subtotal, tax, total],
    );
  }
  ...
  async listHeaders(
    ...
    let i = 1;
    if (query.supplierId) {
      conds.push(`po.supplier_id = $${i++}`);
      args.push(query.supplierId);
    }
    if (query.status) {
    ...
    const [rows, count] = await Promise.all([
      client.query<PoHeaderRow>(
        `${HEADER_SELECT} ${where} ORDER BY po.order_date DESC LIMIT $${i} OFFSET $${i + 1}`,
        [...args, query.pageSize, offset],
      ),
      client.query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM purchase_orders po ${where}`,
        args,
      ),
    ]);
  ...
  async setStatus(client: PoolClient, poId: string, status: string): Promise<void> {
    await client.query(`UPDATE purchase_orders SET status = $2, updated_at = NOW() WHERE id = $1`, [
      poId,
      status,
    ]);
  }
  ...
  async setApprovalId(client: PoolClient, poId: string, approvalId: string): Promise<void> {
    await client.query(`UPDATE purchase_orders SET approval_id = $2 WHERE id = $1`, [
      poId,
      approvalId,
    ]);
  }
  ...
  async setCancelled(client: PoolClient, poId: string, reason: string): Promise<void> {
    await client.query(
      `UPDATE purchase_orders SET status = 'cancelled', cancel_reason = $2, updated_at = NOW() WHERE id = $1`,
      [poId, reason],
    );
  }

  async setPaymentVerificationId(client: PoolClient, poId: string, pvId: string): Promise<void> {
    await client.query(`UPDATE purchase_orders SET payment_verification_id = $2 WHERE id = $1`, [
      poId,
      pvId,
    ]);
  }
  ...
  async incrementLineReceived(client: PoolClient, lineId: string, qty: Qty): Promise<void> {
    await client.query(`UPDATE po_lines SET qty_received = qty_received + $2 WHERE id = $1`, [
      lineId,
      qty,
    ]);
  }
  ...
  async insertReceipt(
    client: PoolClient,
    input: { receiptNumber: string; poId: UUID; receivedBy: UUID; notes: string | null },
  ): Promise<string> {
    const res = await client.query<{ id: string }>(
  ...
  async getItemCosting(
    client: PoolClient,
    itemId: UUID,
  ): Promise<{ name: string; storageType: string; avgCost: Money; qtyOnHand: Qty }> {
    ...
    return {
      ...
      avgCost: row.avg_cost as Money,
      qtyOnHand: row.qty_on_hand as Qty,
    };
  }
  ...
  async storageAreaCheck(
    client: PoolClient,
    storageAreaId: UUID,
  ): Promise<{ type: string; name: string } | undefined> {
    const res = await client.query<{ type: string; name: string }>(
