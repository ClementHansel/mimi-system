 ...
 *
 * BE-TXN-ROLLBACK: `settings.service.ts` (`putOne`, `putApprovalChain`,
 * `putApprovalMode`) and `statutory.service.ts` (`putBpjs`, `putTer`,
 * `putPtkp`, `putArticle17`, `putTaxProfile`, `enable`, `disable`) ran their
 * writes directly on `req.dbClient` with no `COMMIT` anywhere in this
 * module — `RlsCleanupInterceptor`'s unconditional post-request `ROLLBACK`
 * therefore discarded every one of them silently. Fixed by wrapping every
 * mutating method's body in `withWrite`, matching
