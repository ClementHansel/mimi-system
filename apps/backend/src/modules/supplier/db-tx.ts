 ...
 *
 * BE-TXN-ROLLBACK: `supplier.service.ts` (`create`, `update`, `deactivate`,
 * `upsertItem`, `deleteItem`) ran its writes directly on `req.dbClient` with
 * no `COMMIT` anywhere in this module — `RlsCleanupInterceptor`'s
 * unconditional post-request `ROLLBACK` therefore discarded every one of
 * them silently. Fixed by wrapping every mutating method's body in
 * `withWrite`, matching
