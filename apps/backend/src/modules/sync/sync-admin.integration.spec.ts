...
function buildController(_client: PoolClient): SyncAdminController {
  ...
  return new SyncAdminController(
    new SyncConflictsRepository(),
    undefined as unknown as ReconciliationService,
    undefined as never,
  );
}
...
beforeAll(async () => {
  ...
  if (!locRes.rows[0] || !itemRes.rows[0])
    throw new Error('Test fixture requires at least one seeded location and item');
...
describe('SyncAdminController.dismissConflict — write-then-read-back across SEPARATE connections', () => {
  it('persists past its own request — a later read (new connection) sees it dismissed, not open', async () => {
    ...
    try {
      const dismissed = await asRequest((client) =>
        buildController(client).dismissConflict(fakeReq(client, ownerUserId), conflictId, {
          reason: 'test dismiss',
        }),
      );
      ...
      const reread = await withRollback((client) =>
        client.query<{ status: string; resolution: string | null }>(
  ...
  it('rejects a domain-resolved kind (e.g. double_count) with ERR_RESOLVE_IN_DOMAIN and never commits anything', async () => {
    ...
    try {
      ...
      const reread = await withRollback((client) =>
        client.query<{ status: string }>(`SELECT status FROM sync_conflicts WHERE id = $1`, [
          conflictId,
        ]),
      );
...
describe('SyncAdminController.resolveReconciliation — write-then-read-back across SEPARATE connections', () => {
  it('persists past its own request — a later read (new connection) sees it resolved, not open', async () => {
    ...
    try {
      const resolved = await asRequest((client) =>
        buildController(client).resolveReconciliation(
          fakeReq(client, ownerUserId),
          reconciliationId,
          { resolution: 'adjusted via test' },
        ),
      );
      ...
      const reread = await withRollback((client) =>
        client.query<{ status: string; resolution: string | null }>(
          `SELECT status, resolution FROM stock_reconciliations WHERE id = $1`,
          [reconciliationId],
        ),
      );
      ...
      await getOwnerPool().query(`DELETE FROM stock_reconciliations WHERE id = $1`, [
        reconciliationId,
      ]);
    }
  });
  ...
  it('404s when the reconciliation is already resolved (not open) and never re-commits', async () => {
    ...
    try {
      ...
      const reread = await withRollback((client) =>
        client.query<{ resolution: string | null }>(
          `SELECT resolution FROM stock_reconciliations WHERE id = $1`,
          [reconciliationId],
        ),
      );
