...
export class ReturnService {
  ...
  async create(
    ...
    if (dto.direction === ReturnDirection.OUTLET_TO_WAREHOUSE && !dto.toLocationId) {
      throw new BadRequestException({
        code: ERR_VALIDATION,
        message: 'toLocationId is required for outlet_to_warehouse returns',
      });
    }
    if (dto.direction === ReturnDirection.WAREHOUSE_TO_SUPPLIER && !dto.supplierId) {
      throw new BadRequestException({
        code: ERR_VALIDATION,
        message: 'supplierId is required for warehouse_to_supplier returns',
      });
    }
    if (dto.photoAttachmentIds.length === 0) {
      throw new BadRequestException({
        code: ERR_PHOTO_REQUIRED,
        message: 'At least one photo is wajib for a return',
      });
    }
    if (dto.lines.some((l) => !l.reason?.trim())) {
      throw new BadRequestException({
        code: ERR_VALIDATION,
        message: 'Every return line needs a reason (FR-WST-01)',
      });
    }
    ...
    const duplicateItemIds = dto.lines
      .map((l) => l.itemId)
      .filter((itemId, idx, all) => all.indexOf(itemId) !== idx);
    ...
    return withWrite(client, async () => {
      ...
      for (const line of dto.lines) {
        const itemRes = await client.query<{ avg_cost: string }>(
      ...
      for (const attachmentId of dto.photoAttachmentIds) {
        await client.query(
          `UPDATE attachments SET entity_type = 'return', entity_id = $2 WHERE id = $1 AND entity_id IS NULL`,
          [attachmentId, id],
        );
      }
  ...
  async submit(client: PoolClient, actor: ActorContext, id: UUID): Promise<ReturnDetail> {
    ...
    if (header.status !== ReturnStatus.DRAFT) {
      throw new ConflictException({
        code: ERR_CONFLICT,
        message: `Return ${id} is '${header.status}', not 'draft'`,
      });
    }
    ...
    return withWrite(client, async () => {
      ...
      await this.sync.emit(client, {
        ...
        data: {
          ...
          toLocationId: header.to_location_id ?? undefined,
  ...
  async approve(
    ...
    if (header.status !== ReturnStatus.SUBMITTED) {
      throw new ConflictException({
        code: ERR_CONFLICT,
        message: `Return ${id} is '${header.status}', not 'submitted'`,
      });
    }
  ...
  async ship(
    ...
    if (header.status !== ReturnStatus.APPROVED) {
      throw new ConflictException({
        code: ERR_CONFLICT,
        message: `Return ${id} is '${header.status}', not 'approved'`,
      });
    }
    if (dto.proofAttachmentIds.length === 0) {
      throw new BadRequestException({
        code: ERR_PHOTO_REQUIRED,
        message: 'At least one shipping proof photo is wajib (FR-WST-03)',
      });
    }
    ...
    return withWrite(client, async () => {
      ...
      for (const attachmentId of dto.proofAttachmentIds) {
        await client.query(
          `UPDATE attachments SET entity_type = 'return', entity_id = $2, kind = 'return_proof' WHERE id = $1`,
          [attachmentId, id],
        );
      }
  ...
  async receive(
    ...
    if (header.direction !== ReturnDirection.OUTLET_TO_WAREHOUSE) {
      throw new BadRequestException({
        code: ERR_VALIDATION,
        message: 'Only outlet_to_warehouse returns are received at the warehouse',
      });
    }
    if (header.status !== ReturnStatus.IN_TRANSIT) {
      throw new ConflictException({
        code: ERR_CONFLICT,
        message: `Return ${id} is '${header.status}', not 'in_transit'`,
      });
    }
    if (dto.proofAttachmentIds.length === 0) {
      throw new BadRequestException({
        code: ERR_PHOTO_REQUIRED,
        message: 'At least one receiving proof photo is wajib',
      });
    }
    ...
    return withWrite(client, async () => {
      ...
      for (const line of dto.lines) {
        ...
        if (!rl)
          throw new NotFoundException({
            code: ERR_NOT_FOUND,
            message: `Return line ${line.lineId} not found on return ${id}`,
          });
        if (isNegativeQty(line.qtyReceived)) {
          throw new BadRequestException({
            code: ERR_VALIDATION,
            message: `qtyReceived must be >= 0 for line ${line.lineId}`,
          });
        }
      ...
      for (const attachmentId of dto.proofAttachmentIds) {
        await client.query(
          `UPDATE attachments SET entity_type = 'return', entity_id = $2, kind = 'receiving_photo' WHERE id = $1`,
          [attachmentId, id],
        );
      }
  ...
  async complete(client: PoolClient, id: UUID, dto: CompleteReturnDto): Promise<ReturnDetail> {
    ...
    if (header.status !== expectedFromStatus) {
      throw new ConflictException({
        code: ERR_CONFLICT,
        message: `Return ${id} is '${header.status}', not '${expectedFromStatus}'`,
      });
    }

    return withWrite(client, async () => {
      if (header.direction === ReturnDirection.WAREHOUSE_TO_SUPPLIER && dto.creditNoteRef) {
        await client.query(`UPDATE returns SET notes = COALESCE(notes, '') || $2 WHERE id = $1`, [
          id,
          ` [credit note: ${dto.creditNoteRef}]`,
        ]);
      }
  ...
  private async requireHeader(client: PoolClient, id: UUID): Promise<ReturnHeaderRow> {
    ...
    if (!header)
      throw new NotFoundException({ code: ERR_NOT_FOUND, message: `Return ${id} not found` });
    return header;
  }
