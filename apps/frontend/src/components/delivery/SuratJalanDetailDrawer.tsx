 ...
 */
export function SuratJalanDetailDrawer({
  id,
  onClose,
  onChanged,
}: {
    ...
    <Drawer open onClose={onClose} title={sj?.sjNumber ?? t('delivery.detail.title')} size="lg">
        ...
        <div className="flex flex-col gap-6">
          <section className="flex flex-col gap-2">
            ...
            <p className="text-sm text-text-muted">
              {sj.vehicle.plateNumber} — {sj.driver.name} · {t('delivery.columnPlannedDate')}:{' '}
              {sj.plannedDate}
            </p>
          </section>

          {/*
            Route planning sits ABOVE the read-only drop list: the dispatcher's
            job on this screen is to decide the sequence and brief, and the
            per-drop execution detail below is the record of what then happened.
            Gated on `delivery.sj.create`, matching the endpoint.
          */}
          <PermissionGate permission="delivery.sj.create">
            <section className="border-t border-border pt-4">
              <RoutePlanner
                sjId={id}
                drops={sj.drops}
                editable={sj.status === 'draft' || sj.status === 'ready'}
                onSaved={load}
              />
            </section>
          </PermissionGate>
          ...
          <section className="flex flex-col gap-3 border-t border-border pt-4">
            <h3 className="text-sm font-semibold text-text-primary">
              {t('delivery.detail.drops')}
            </h3>
            {sj.drops.map((drop) => (
              <DropCard
                key={drop.id}
                drop={drop}
                tempLogs={sj.tempLogs.filter((l) => l.dropId === drop.id)}
              />
            ))}
          </section>
          ...
          {sj.tempLogs.filter((l) => l.dropId === null).length > 0 && (
            <section className="flex flex-col gap-2 border-t border-border pt-4">
              <h3 className="flex items-center gap-1.5 text-sm font-semibold text-text-primary">
                ...
                {t('delivery.detail.coldChain')} — {t('warehouse.sj.stage.load')}
              </h3>
              {sj.tempLogs
                .filter((l) => l.dropId === null)
                .map((log) => (
            ...
            <PermissionGate permission="delivery.sj.create">
              {sj.status === 'draft' && (
                <Button
                  ...
                  size="sm"
                  loading={acting}
                  onClick={() => run(() => readySuratJalan(id), undefined)}
                >
            ...
            <PermissionGate permission="delivery.sj.dispatch">
              ...
              {sj.status === 'loading' && (
                <Button
                  ...
                  loading={acting}
                  onClick={() => run(() => dispatchSuratJalan(id), 'warehouse.sj.dispatched')}
                >
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            {seals.map((s, idx) => (
              <div key={idx} className="flex gap-2">
                <Input
                  value={s}
                  onChange={(e) =>
                    setSeals((prev) => prev.map((v, i) => (i === idx ? e.target.value : v)))
                  }
                ...
                {seals.length > 1 && (
                  <Button
                    ...
                    variant="ghost"
                    size="sm"
                    onClick={() => setSeals((prev) => prev.filter((_, i) => i !== idx))}
                  >
            ...
            <Button
              ...
              onClick={() => setSeals((prev) => [...prev, ''])}
            >
              {t('warehouse.sj.addSeal')}
            </Button>
          </div>
          {sj?.shipmentType === 'frozen' && (
            <TempInput
              label={t('warehouse.sj.loadTemp')}
              required
              value={loadTemp}
              onChange={setLoadTemp}
            />
          )}
        </div>
        <div className="mt-4 flex justify-end gap-2">
          ...
          <Button loading={acting} onClick={submitLoad}>
            {t('common.submit')}
          </Button>
        </div>
      </Modal>
    ...
    <div className="rounded-lg border border-border p-3">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
        <p className="font-medium text-text-primary">
          {t('delivery.detail.dropSeq', { seq: drop.dropSeq, location: drop.locationName })}
        </p>
      ...
      <div className="mb-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-muted">
        <span>
          {t('delivery.detail.dropDeparted')}:{' '}
          {drop.departedAt ? fmtDateTime(drop.departedAt) : '—'}
        </span>
        <span>
          {t('delivery.detail.dropArrived')}: {drop.arrivedAt ? fmtDateTime(drop.arrivedAt) : '—'}
        </span>
        <span>
          {t('delivery.detail.dropReceived')}:{' '}
          {drop.receivedBy ? `${drop.receivedBy} (${fmtDateTime(drop.receivedAt)})` : '—'}
        </span>
      </div>
      ...
      {drop.status === 'completed_discrepancy' && (
        <div className="mb-2 flex items-center gap-1.5 rounded bg-warning-50 px-2 py-1 text-xs text-warning-700">
          ...
          {t('delivery.detail.dropDiscrepancy')}
          {drop.discrepancyNotes ? `: ${drop.discrepancyNotes}` : ''}
        </div>
      )}
          ...
          <tr className="text-left text-xs text-text-muted">
            ...
            {isDropTerminal(drop.status) && (
              <th className="pb-1 text-right font-normal">
                {t('delivery.detail.lineQtyReceived')}
              </th>
            )}
          </tr>
        </thead>
        <tbody>
          {drop.lines.map((l) => (
            <tr key={l.id} className="border-t border-border">
              ...
              {isDropTerminal(drop.status) && (
                <td
                  className={`py-1.5 text-right tabular-nums ${l.discrepancyReason ? 'text-warning-700' : ''}`}
                >
...
function TempLogRow({ log }: { log: TempLog }) {
  ...
  return (
    <div
      className={`flex flex-wrap items-center gap-2 text-xs ${log.isBreach ? 'text-danger-600' : 'text-text-secondary'}`}
    >
      ...
      {log.isBreach && (
        <Badge variant="danger" size="sm">
          ...
          {t('delivery.detail.coldChainBreach', {
            class: (log.breachedClasses ?? []).join(', ') || '—',
          })}
        </Badge>
      )}
