...
export function DocCard({ manual }: { manual: DocManual }) {
  ...
  return (
    ...
    >
      <Badge variant="brand" size="sm" className="self-start">{manual.audience}</Badge>
      ...
      <div className="mt-auto flex items-center gap-3 pt-2 text-xs text-text-muted">
        ...
        <span className="inline-flex items-center gap-1">
          <FileText className="size-3.5" aria-hidden /> {t('docs.sectionsCount', { count: manual.sections.length })}
        </span>
      </div>
    </Link>
  );
