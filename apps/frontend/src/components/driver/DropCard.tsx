...
export function DropCard({ sj, drop, onChanged }: DropCardProps) {
  ...
  return (
    <Card className={cn(action === 'none' && 'opacity-90')}>
      <CardContent className="flex flex-col gap-3">
        ...
        {(drop.departedAt || drop.arrivedAt || drop.receivedAt) && (
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-text-muted">
            {drop.departedAt && <span>{t('driver.actions.depart')}: {fmtTime(drop.departedAt)}</span>}
            {drop.arrivedAt && <span>{t('driver.actions.arrive')}: {fmtTime(drop.arrivedAt)}</span>}
            {drop.receivedAt && <span>{t('driver.actions.receive')}: {fmtTime(drop.receivedAt)}</span>}
          </div>
        )}
      ...
      {modal === 'depart' && <DropDepartModal open sj={sj} drop={drop} onClose={() => setModal(null)} onDone={handleDone} />}
      {modal === 'arrive' && <DropArriveModal open sj={sj} drop={drop} onClose={() => setModal(null)} onDone={handleDone} />}
      {modal === 'receive' && <DropReceiveModal open sj={sj} drop={drop} onClose={() => setModal(null)} onDone={handleDone} />}
      {modal === 'fail' && <DropFailModal open drop={drop} onClose={() => setModal(null)} onDone={handleDone} />}
    </Card>
  );
}
