...
export function ChartOfAccountsPanel() {
  ...
  const columns: DataTableColumn<Account>[] = [
    ...
    {
      key: 'type',
      header: t('finance.coa.columnType'),
      render: (r) => t(`finance.accountType.${r.type}`),
    },
    {
      key: 'normalBalance',
      header: t('finance.coa.columnNormalBalance'),
      render: (r) =>
        t(
          r.normalBalance === 'debit'
            ? 'finance.coa.normalBalanceDebit'
            : 'finance.coa.normalBalanceCredit',
        ),
    },
    {
      key: 'isPostable',
      header: t('finance.coa.columnPostable'),
      render: (r) => (r.isPostable ? t('common.yes') : t('common.no')),
    },
    {
      key: 'isActive',
      header: t('finance.coa.columnStatus'),
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div className="flex flex-wrap items-end gap-2">
          <Input
            placeholder={t('finance.coa.searchPlaceholder')}
            value={q}
          ...
          <Select
            ...
            placeholder={t('finance.coa.filterTypeAll')}
          ...
          <Select
            ...
            placeholder={t('finance.coa.filterStatusAll')}
            options={[
...
function AccountFormModal({
  ...
  async function submit() {
    ...
    try {
      ...
      toast({
        title: t(account ? 'finance.coa.updateSuccess' : 'finance.coa.createSuccess'),
        variant: 'success',
      });
      onSaved();
    } catch (err) {
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={t(account ? 'finance.coa.editTitle' : 'finance.coa.createTitle')}
      footer={
        <>
    ...
    >
      <div className="grid grid-cols-2 gap-3">
        ...
        />
        <Input
          label={t('finance.coa.name')}
          value={name}
          onChange={(e) => setName(e.target.value)}
          required
        />
        <Select
          label={t('finance.coa.type')}
          value={type}
          onValueChange={setType}
          disabled={!!account}
          options={TYPE_OPTIONS.map((v) => ({ value: v, label: t(`finance.accountType.${v}`) }))}
        />
        <Select
          label={t('finance.coa.columnNormalBalance')}
          value={normalBalance}
          onValueChange={(v) => setNormalBalance(v as 'debit' | 'credit')}
          disabled={!!account}
          options={[
            { value: 'debit', label: t('finance.coa.normalBalanceDebit') },
            { value: 'credit', label: t('finance.coa.normalBalanceCredit') },
          ]}
        />
        {!account && (
          <Select
            label={t('finance.coa.parent')}
            value={parentId}
        ...
        {account ? (
          ...
          <div className="col-span-2">
            <Checkbox
              label={t('finance.coa.columnPostable')}
              checked={isPostable}
