...
export async function uploadSelfie(file: File): Promise<string> {
  const presign = await api.post<{ attachmentId: string; uploadUrl: string; objectKey: string; expiresAt: string }>(
    '/attachments/presign',
    { fileName: file.name, mimeType: file.type, sizeBytes: file.size, kind: 'selfie' },
  );

  const target = /^https?:\/\//i.test(presign.uploadUrl)
    ? presign.uploadUrl
    : `${API_BASE_URL}${presign.uploadUrl.startsWith('/') ? '' : '/'}${presign.uploadUrl}`;
  const putRes = await fetch(target, { method: 'PUT', body: file, headers: { 'Content-Type': file.type } });
