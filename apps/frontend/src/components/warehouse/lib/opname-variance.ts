 ...
 *
 * Field-for-field identical to `components/outlet/lib/opname-variance.ts` —
 * kept as its own copy here rather than a cross-surface import, matching the
 * outlet/warehouse ownership split `lib/types.ts`'s header comment already
 * documents for wire shapes (each surface owns its own `lib/`).
 */
