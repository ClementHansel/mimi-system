...
async function main(): Promise<void> {
  ...
  try {
    ...
    async function upsertLocation(
      ...
      const res = await client.query(
        ...
        [
          ...
          type,
          city,
          `Jl. ${name}, ${city}, Kalimantan`,
          `0542${rnd(1000000, 9999999)}`,
          lat,
          lng,
        ],
      );
    ...
    locationId['GDG'] = await upsertLocation(
      'GDG',
      'Gudang Pusat Balikpapan',
      'warehouse',
      'Balikpapan',
      gdgLat,
      gdgLng,
    ...
    for (const city of Object.keys(cityDistricts)) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        locationId[code] = await upsertLocation(
          code,
          `Mimi Chicken ${districts[i]}`,
          'outlet',
          city,
          jitterLat,
          jitterLng,
    ...
    console.log(
      `  - locations: 1 gudang + ${allLocationCodes.length - 1} outlets across ${Object.keys(cityDistricts).length} cities`,
    );
    ...
    async function upsertStorageArea(
      locCode: string,
      code: string,
      name: string,
      type: string,
    ...
    for (const code of allLocationCodes) {
      ...
      seedUsers.push({
        username: `spv_${code.toLowerCase()}`,
        name: randomName(),
        roleKey: 'supervisor',
        locations: [code],
        withPin: true,
      ...
      seedUsers.push({
        username: `ldr_${code.toLowerCase()}`,
        name: randomName(),
        roleKey: 'leader_outlet',
        locations: [code],
      });
      seedUsers.push({
        username: `kasir1_${code.toLowerCase()}`,
        name: randomName(),
        roleKey: 'kasir',
        locations: [code],
      });
      seedUsers.push({
        username: `kasir2_${code.toLowerCase()}`,
        name: randomName(),
        roleKey: 'kasir',
        locations: [code],
      });
    ...
    for (const u of seedUsers) {
      const res = await client.query(
        ...
        [
          u.username,
          `${u.username}@mimichicken.local`,
          passwordHash,
          u.withPin ? pinHash : null,
          u.name,
          roleId[u.roleKey],
        ],
      );
    ...
    console.log(
      `  - users: ${seedUsers.length} across 9 roles (login: <username> / ${DEMO_PASSWORD}, PIN ${DEMO_PIN} where applicable)`,
    );
    ...
    async function upsertEmployee(
      ...
      const res = await client.query(
        ...
        [
          ...
          userId,
          name,
          `64${rnd(1000000000000, 9999999999999)}`,
          `08${rnd(100000000, 999999999)}`,
          isoDate(daysAgo(joinDaysAgo)),
          position,
          locationId[locCode],
        ],
      );
      ...
      if (!id) {
        id = (
          await client.query('SELECT id FROM employees WHERE employee_number = $1', [
            employeeNumber,
          ])
        ).rows[0].id;
      }
    ...
    for (let i = 1; i <= driverCount; i++) {
      ...
      const res = await client.query(
        ...
        [
          ...
          seedUsers.find((u) => u.username === username)!.name,
          `08${rnd(100000000, 999999999)}`,
          `SIM${rnd(100000, 999999)}`,
        ],
      );
    ...
    for (let i = 0; i < vehiclePlates.length; i++) {
      ...
      const res = await client.query(
        ...
        [
          vehiclePlates[i],
          hasFreezer ? 'truck' : 'van',
          pick(['Mitsubishi', 'Isuzu', 'Toyota']),
          pick(['Colt Diesel', 'Elf', 'HiAce']),
          hasFreezer,
        ],
      );
      vehicleId[vehiclePlates[i]] =
        res.rows[0]?.id ??
        (await client.query('SELECT id FROM vehicles WHERE plate_number=$1', [vehiclePlates[i]]))
          .rows[0].id;
    }
    ...
    for (const name of categoryNames) {
      ...
      categoryId[name] =
        res.rows[0]?.id ??
        (await client.query('SELECT id FROM item_categories WHERE name=$1', [name])).rows[0].id;
    }
    ...
    for (const [code, name] of unitDefs) {
      ...
      unitId[code] =
        res.rows[0]?.id ??
        (await client.query('SELECT id FROM units WHERE code=$1', [code])).rows[0].id;
    }
    ...
    [
      ...
      'Ayam Paha Bawah',
      'Ayam Sayap',
      'Ayam Fillet Berbumbu Original',
      'Ayam Fillet Berbumbu Pedas',
      'Ayam Wing Berbumbu',
      'Ayam Drumstick Berbumbu',
      'Ayam Karkas',
      'Ayam Fillet Berbumbu Extra Crispy',
      'Ayam Paha Berbumbu Madu',
      'Ayam Potong Kampung',
      'Ayam Fillet Tanpa Tulang',
      'Ayam Giling',
    ].forEach((n) => addItem(n, 'Ayam Mentah', 'kg', 'frozen', rnd(28000, 42000)));
    // Bumbu
    [
    ...
    // Daging Olahan
    ['Sosis Ayam', 'Nugget Ayam', 'Bakso Ayam', 'Kornet Ayam'].forEach((n) =>
      addItem(n, 'Daging Olahan', 'kg', 'frozen', rnd(30000, 55000)),
    );
    ...
    for (const d of itemDefs) {
      ...
      itemId[d.name] =
        res.rows[0]?.id ??
        (await client.query('SELECT id FROM items WHERE sku=$1', [d.sku])).rows[0].id;
    }
    console.log(`  - items: ${itemDefs.length}`);

    // generic unit conversions
    const genericConversions: [string, string, number][] = [
      ['kg', 'gr', 1000],
      ['ltr', 'ml', 1000],
      ['box', 'pcs', 50],
      ['pack', 'pcs', 12],
    ...
    addProduct('Wing Crispy', 'Ayam', 10000, [
      'Ayam Wing Berbumbu',
      'Tepung Bumbu Original',
      'Minyak Goreng Kemasan',
    ]);
    addProduct('Drumstick Madu', 'Ayam', 14000, [
      'Ayam Paha Berbumbu Madu',
      'Minyak Goreng Kemasan',
    ]);
    ...
    addProduct('Paket Hemat Berdua', 'Paket', 38000, [
      'Beras Premium',
      'Ayam Potong Utuh',
      'Tepung Bumbu Original',
      'Box Nasi Besar',
    ]);
    addProduct('Paket Nasi + Wing', 'Paket', 19000, [
      'Beras Premium',
      'Ayam Wing Berbumbu',
      'Box Nasi Sedang',
    ]);
    addProduct('Paket Keluarga (4 potong)', 'Paket', 65000, [
      'Ayam Potong Utuh',
      'Tepung Bumbu Original',
      'Beras Premium',
      'Box Nasi Besar',
    ...
    addProduct('Es Leci', 'Minuman', 7000, [
      'Sirup Leci',
      'Selasih',
      'Es Batu Kristal',
      'Cup Minuman 16oz',
    ]);
    addProduct('Es Lemon Tea', 'Minuman', 7000, [
      'Bubuk Lemon Tea',
      'Es Batu Kristal',
      'Cup Minuman 16oz',
    ]);
    addProduct('Kopi Susu Dingin', 'Minuman', 9000, [
      'Kopi Bubuk',
      'Susu Kental Manis',
      'Es Batu Kristal',
      'Cup Minuman 16oz',
    ...
    addProduct('Ayam Goreng Original 2pc', 'Ayam', 22000, [
      'Ayam Potong Utuh',
      'Tepung Bumbu Original',
      'Minyak Goreng Kemasan',
    ]);
    addProduct('Ayam Goreng Pedas 2pc', 'Ayam', 24000, [
      'Ayam Potong Utuh',
      'Tepung Bumbu Pedas',
      'Minyak Goreng Kemasan',
    ]);
    addProduct('Ayam Geprek Sambal Matah', 'Ayam', 17000, [
      fillet,
      'Sambal Matah',
      'Minyak Goreng Kemasan',
    ]);
    addProduct('Paha Atas Crispy', 'Ayam', 13000, [
      'Ayam Paha Atas',
      'Tepung Bumbu Original',
      'Minyak Goreng Kemasan',
    ]);
    addProduct('Paha Bawah Crispy', 'Ayam', 11000, [
      'Ayam Paha Bawah',
      'Tepung Bumbu Original',
      'Minyak Goreng Kemasan',
    ]);
    addProduct('Dada Crispy', 'Ayam', 14000, [
      'Ayam Fillet Dada',
      'Tepung Bumbu Original',
      'Minyak Goreng Kemasan',
    ]);
    addProduct('Paket Ulang Tahun (8 potong)', 'Paket', 130000, [
      'Ayam Potong Utuh',
      'Tepung Bumbu Original',
      'Beras Premium',
      'Box Nasi Besar',
    ]);
    addProduct('Rice Bowl Geprek', 'Paket', 21000, [
      'Beras Premium',
      fillet,
      'Sambal Bawang',
      'Box Nasi Kecil',
    ...
    // =========================================================================
    const supplierDefs = [
      'CV Ayam Segar Kaltim',
      'PT Sumber Protein Nusantara',
      'UD Bumbu Rasa Kalimantan',
      'CV Kemasan Jaya Abadi',
      'PT Sembako Borneo Makmur',
      'Toko Minyak Goreng Sejahtera',
      'CV Beras Kalimantan Timur',
      'UD Sayur Segar Balikpapan',
      'PT Distribusi Minuman Kaltim',
      'CV Bumbu Dapur Nusantara',
      'Toko Grosir Sembako Samarinda',
      'PT Kemasan Plastik Prima',
      'UD Ayam Potong Berkah',
      'CV Rempah Kalimantan',
      'PT Cold Chain Logistik Borneo',
    ];
    ...
    for (let i = 0; i < supplierDefs.length; i++) {
      ...
      const res = await client.query(
        ...
        [
          ...
          randomName(),
          `08${rnd(100000000, 999999999)}`,
          `${code.toLowerCase()}@supplier.local`,
          `Jl. Industri No. ${rnd(1, 99)}, Balikpapan`,
          pick([0, 7, 14, 30]),
          pick(['BCA', 'Mandiri', 'BNI', 'BRI']),
          `${rnd(1000000000, 9999999999)}`,
          supplierDefs[i],
          outletVisible,
        ],
      );
      supplierId[supplierDefs[i]] =
        res.rows[0]?.id ??
        (await client.query('SELECT id FROM suppliers WHERE code=$1', [code])).rows[0].id;
    }
    ...
    for (const itemName of itemNames) {
      ...
      supIdx++;
      const price =
        itemDefs.find((d) => d.name === itemName)!.avgCost * (0.85 + Math.random() * 0.2);
      await client.query(
      ...
      for (let h = 0; h < 3; h++) {
        await client.query(
          ...
          [
            supplierId[supName],
            itemId[itemName],
            (price * (0.92 + h * 0.04)).toFixed(2),
            isoDate(daysAgo(90 - h * 30)),
          ],
        );
      }
    }
    ...
    for (const code of allLocationCodes) {
      ...
      for (const itemName of coreItems) {
        ...
        const areaType =
          def.storageType === 'frozen'
            ? 'freezer'
            : def.storageType === 'chilled'
              ? 'chiller'
              : 'dry_store';
    ...
    console.log(
      `  - min-stock rules + opening stock balances for ${coreItems.length} core items across all locations`,
    );
      ...
      for (let day = 6; day >= 0; day--) {
        ...
        const shiftRes = await client.query(
          ...
          [
            ...
            kasirId,
            openedAt,
            isToday ? null : kasirId,
            isToday ? null : closedAt,
            isToday ? null : rnd(800000, 2500000),
            isToday ? 'open' : 'closed',
            clientId,
          ],
        );
        const shiftId =
          shiftRes.rows[0]?.id ??
          (await client.query('SELECT id FROM pos_shifts WHERE client_id=$1', [clientId])).rows[0]
            .id;
        ...
        for (let s = 0; s < salesToday; s++) {
          ...
          const saleRes = await client.query(
            `INSERT INTO sales (receipt_number, client_id, location_id, shift_id, kasir_id, status, subtotal, discount, total, paid_amount, change_amount, occurred_at)
             ...
             ON CONFLICT (client_id) DO NOTHING RETURNING id`,
            [
              `${code}-${isoDate(openedAt).replace(/-/g, '')}-${String(s + 1).padStart(3, '0')}`,
              saleClientId,
              locationId[code],
              shiftId,
              kasirId,
          ...
          // Stock depletion for these sales is NOT done here — it runs as a
          // backfill pass in seed-extended.ts, keyed on whether a sale already
          // has usage_out movements. Doing it inline would only ever fire for
          // NEWLY inserted sales, so every database seeded before this change
          // (including production) would keep its untouched opening balances
          // and its permanently empty mv_item_usage_daily forever.
    ...
    // One cash-variance proposal (Amendment 2) on a closed shift with a shortfall
    {
      ...
      if (shift.rows[0]) {
        ...
        await client.query(`UPDATE pos_shifts SET cash_variance = -($1::numeric) WHERE id=$2`, [
          shortfall,
          s.id,
        ]);
        await client.query(
    ...
    for (const state of rrStates) {
      ...
      const already = await client.query(
        'SELECT 1 FROM replenishment_requests WHERE client_id = $1',
        [clientId],
      );
      if (already.rows.length > 0) continue;
      ...
      const res = await client.query(
        ...
        [
          ...
          state,
          requestedBy,
          state === 'draft' ? null : daysAgo(rnd(1, 10)),
          isoDate(daysAgo(-3)),
          clientId,
        ],
      );
      ...
      if (state === 'rejected') {
        await client.query(`UPDATE replenishment_requests SET rejection_reason=$1 WHERE id=$2`, [
          'Stok gudang tidak mencukupi minggu ini',
          rrId,
        ]);
      }
    }
    ...
    // =========================================================================
    {
      ...
      const existingSj = await client.query('SELECT id FROM surat_jalan WHERE notes = $1', [
        sjMarker,
      ]);
      let sjId: string;
      let isNewSj = false;
      if (existingSj.rows.length > 0) {
        ...
        const sjRes = await client.query(
          ...
          [
            sjNumber,
            locationId['GDG'],
            driverRowId[driverUsername],
            Object.values(vehicleId)[3],
            isoDate(daysAgo(0)),
            daysAgo(0),
      ...
      for (let i = 0; i < sjOutlets.length; i++) {
        ...
        const dropRes = await client.query(
          ...
          [
            ...
            status === 'completed' ? daysAgo(0) : null,
            dropClientId,
          ],
        );
        const dropId =
          dropRes.rows[0]?.id ??
          (await client.query('SELECT id FROM sj_drops WHERE client_id=$1', [dropClientId])).rows[0]
            .id;
    ...
    for (const code of demoOutlets) {
      for (const roleUser of [
        `spv_${code.toLowerCase()}`,
        `kasir1_${code.toLowerCase()}`,
        `kasir2_${code.toLowerCase()}`,
      ]) {
        const empId = employeeIdByUsername[roleUser];
        if (!empId) continue;
        const [lat, lng] =
          cityCoords[Object.keys(cityDistricts).find((c) => outletCodesByCity[c].includes(code))!];
    ...
    await client.query(
      ...
      [
        employeeIdByUsername['kasir1_' + demoOutlets[0].toLowerCase()],
        isoDate(daysAgo(-10)),
        isoDate(daysAgo(-9)),
      ],
    );
    ...
    const periodId =
      periodRes.rows[0]?.id ??
      (await client.query('SELECT id FROM payroll_periods WHERE period_code=$1', [periodCode]))
        .rows[0].id;
    ...
    const existingRun = await client.query(
      'SELECT id FROM payroll_runs WHERE period_id = $1 AND run_seq = 1',
      [periodId],
    );
    let runId: string;
    if (existingRun.rows.length > 0) {
      ...
      runId =
        runRes.rows[0]?.id ??
        (
          await client.query('SELECT id FROM payroll_runs WHERE period_id = $1 AND run_seq = 1', [
            periodId,
          ])
        ).rows[0].id;
    }
    const baseSalaryComponent = (
      await client.query(`SELECT id FROM salary_components WHERE code='base_salary'`)
    ).rows[0].id;
    const allEmployees = (
      await client.query('SELECT id, employee_number FROM employees ORDER BY employee_number')
    ).rows;
    let totalGross = 0;
    for (const emp of allEmployees) {
      const employment = (
        await client.query(
          'SELECT base_salary, position FROM employments WHERE employee_id=$1 LIMIT 1',
          [emp.id],
        )
      ).rows[0];
    ...
    console.log(
      `  - payroll period ${periodCode} with a calculated run (${allEmployees.length} lines)`,
    );
    ...
    // =========================================================================
    {
      ...
      const existingPo = await client.query('SELECT id FROM purchase_orders WHERE notes = $1', [
        poMarker,
      ]);
      ...
      if (poId) {
        ...
        await client.query('UPDATE purchase_orders SET subtotal=$1, total=$1 WHERE id=$2', [
          subtotal,
          poId,
        ]);
      }
    }
    {
      ...
      const existingPc = await client.query('SELECT 1 FROM petty_cash WHERE client_id = $1', [
        pcClientId,
      ]);
      if (existingPc.rows.length === 0) {
        ...
        await client.query(
          ...
          [
            pcNumber,
            locationId[code],
            userIdByUsername[`ldr_${code.toLowerCase()}`],
            isoDate(daysAgo(1)),
            supplierDefs[0],
            pcClientId,
          ],
    ...
    }
    {
      ...
      const existingWst = await client.query('SELECT 1 FROM waste_records WHERE client_id = $1', [
        wstClientId,
      ]);
      if (existingWst.rows.length === 0) {
        ...
        await client.query(
          ...
          [
            ...
            batchId,
            locationId[code],
            storageAreaId[code]['dry_store'],
            itemId[coreItems[0]],
            itemDefs.find((d) => d.name === coreItems[0])!.avgCost,
            userIdByUsername[`ldr_${code.toLowerCase()}`],
            wstClientId,
          ],
        );
    ...
    for (const code of demoOutlets.slice(0, 3)) {
      for (let i = 0; i < 3; i++) {
        ...
        await client.query(
          ...
          [
            stableUuid(`oo-${code}-${i}`),
            locationId[code],
            i % 2 === 0 ? 'gofood' : 'shopeefood',
            `${code}-ORD-${1000 + i}`,
            isoDate(daysAgo(i)),
            gross,
            fee,
            gross - fee,
            userIdByUsername[`kasir1_${code.toLowerCase()}`],
          ],
        );
      }
    }
    ...
    for (const code of [...demoOutlets.slice(0, 3), 'GDG']) {
      const assetRes = await client.query(
        ...
        [
          `AST${String(assetSeq++).padStart(4, '0')}`,
          code === 'GDG' ? 'Freezer Gudang' : 'AC Ruangan',
          locationId[code],
        ],
      );
    ...
    for (let i = 0; i < targetDevices; i++) {
      ...
      const lastSeen =
        status === 'online' ? new Date() : status === 'stale' ? daysAgo(0) : daysAgo(2 + (i % 18));
    ...
    console.log(
      `  - devices: ${targetDevices} across locations (mixed online/stale/offline/unpaired)`,
    );
    ...
    // never assumed.
    // =========================================================================
    const acctRows = (await client.query('SELECT code, id FROM chart_of_accounts')).rows as {
      code: string;
      id: string;
    }[];
    ...
    const periodRows = (await client.query('SELECT period_code, id FROM fiscal_periods')).rows as {
      period_code: string;
      id: string;
    }[];
    ...
    async function insertJournalEntry(e: SeedEntry): Promise<string> {
      ...
      if (Math.round(totalDebit * 100) !== Math.round(totalCredit * 100)) {
        throw new Error(
          `Seed journal entry '${e.description}' does not balance: debit ${totalDebit} != credit ${totalCredit}`,
        );
      }
      ...
      const res = await client.query(
        ...
        [
          ...
          e.refType,
          e.refId,
          e.locationCode ? locationId[e.locationCode] : null,
          e.description,
          e.source === 'manual' ? financeUserId : null,
        ],
      );
      ...
      for (const line of e.lines) {
        ...
        if (!accountId)
          throw new Error(
            `Seed journal entry '${e.description}' references unknown account code '${line.code}'`,
          );
        await client.query(
    ...
    await insertJournalEntry({
      dateStr: isoDate(daysAgo(20)),
      eventType: 'gudang_purchase',
      source: 'system',
      refType: 'po_receipt',
      refId: null,
      locationCode: 'GDG',
      ...
      lines: [
        { code: '1100', debit: 5_000_000, credit: 0 },
        { code: '2000', debit: 0, credit: 5_000_000 },
      ],
    });
    await insertJournalEntry({
      dateStr: isoDate(daysAgo(14)),
      eventType: 'gudang_goods_out_to_outlet',
      source: 'system',
      refType: 'surat_jalan',
      refId: null,
      locationCode: 'GDG',
      description: 'Barang keluar ke outlet via Surat Jalan',
      lines: [
        { code: '1120', debit: 2_000_000, credit: 0 },
        { code: '1100', debit: 0, credit: 2_000_000 },
      ],
    });
    await insertJournalEntry({
      dateStr: isoDate(daysAgo(13)),
      eventType: 'outlet_goods_in_from_warehouse',
      source: 'system',
      refType: 'sj_drops',
      refId: null,
      locationCode: outletA,
      description: 'Barang diterima outlet dari Surat Jalan',
      lines: [
        { code: '1110', debit: 2_000_000, credit: 0 },
        { code: '1120', debit: 0, credit: 2_000_000 },
      ],
    });
    await insertJournalEntry({
      dateStr: isoDate(daysAgo(3)),
      eventType: 'outlet_sales',
      source: 'system',
      refType: 'sale_day',
      refId: null,
      locationCode: outletA,
      description: 'Penjualan harian outlet (tunai + QRIS)',
      lines: [
    ...
    await insertJournalEntry({
      ...
      refId: null,
      locationCode: outletA,
      description: 'Pemakaian bahan baku harian outlet',
      lines: [
        { code: '5000', debit: 1_800_000, credit: 0 },
        { code: '1110', debit: 0, credit: 1_800_000 },
      ],
    });
    await insertJournalEntry({
      dateStr: isoDate(daysAgo(2)),
      eventType: 'outlet_waste',
      source: 'system',
      refType: 'waste_records',
      refId: null,
      locationCode: outletB,
      description: 'Waste/kerusakan barang outlet',
      lines: [
        { code: '5100', debit: 250_000, credit: 0 },
        { code: '1110', debit: 0, credit: 250_000 },
      ],
    });
    ...
    await insertJournalEntry({
      dateStr: isoDate(daysAgo(15)),
      eventType: 'payroll_accrual',
      source: 'system',
      refType: 'payroll_run',
      refId: null,
      locationCode: null,
      description: 'Akrual gaji periode berjalan',
      lines: [
    ...
    await insertJournalEntry({
      ...
      lines: [
        { code: '2100', debit: 45_000_000, credit: 0 },
        { code: '1020', debit: 0, credit: 45_000_000 },
      ],
    });
    // X6 sale_void_reversal — both legs (revenue+cash-back, HPP+inventory-back) in one entry.
    await insertJournalEntry({
      dateStr: isoDate(daysAgo(1)),
      eventType: 'sale_void_reversal',
      source: 'system',
      refType: 'void_refund',
      refId: null,
      locationCode: outletA,
      description: 'Reversal penjualan (void)',
      lines: [
    ...
    const manualEntryNumber = await insertJournalEntry({
      dateStr: isoDate(daysAgo(4)),
      eventType: null,
      source: 'manual',
      refType: null,
      refId: null,
      locationCode: 'GDG',
      description: 'Pembayaran servis AC gudang',
      lines: [
        { code: '6200', debit: 500_000, credit: 0 },
        { code: '1020', debit: 0, credit: 500_000 },
      ],
    });
    ...
    if (originalId) {
      const reversalNumber = await insertJournalEntry({
        dateStr: isoDate(daysAgo(0)),
        eventType: null,
        source: 'manual',
        refType: null,
        refId: null,
        locationCode: 'GDG',
        description: `Reversal of ${manualEntryNumber}: dicatat dua kali`,
        lines: [
          { code: '1020', debit: 500_000, credit: 0 },
          { code: '6200', debit: 0, credit: 500_000 },
        ],
      });
      ...
      if (reversalId) {
        await client.query(
          `UPDATE journal_entries SET status = 'reversed', reversed_by_entry_id = $2 WHERE id = $1 AND status <> 'reversed'`,
          [originalId, reversalId],
        );
      }
    }
    console.log(
      '  - accounting: 11 seeded journal entries across gudang/outlet/payroll/void/manual (incl. one reversed pair)',
    );

    // Everything seed.ts does not cover — statutory payroll config, the
