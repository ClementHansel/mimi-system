...
| GET    | `/api/approvals/:documentType/:documentId`      | (any; RLS-scoped)                                                    | –                                                                                                       | `ApprovalDetail = {approvalId:UUID; state:ApprovalState; amount:Money\|null; steps:{stepNo:number; approverRole:string; state:ApprovalStepState; actedBy:string\|null; actedAt:ISODateTime\|null; reason:string\|null; offlineAuthorized:boolean; reverificationStatus:ReverificationStatus\|null; actedByName:string\|null}[]; currentStep:number\|null; viewerCanDecide?:boolean}` | FR-LOG-05      |
| POST   | `/api/approvals/:documentType/:documentId/code` | `approval.code.issue` (+ must be an eligible approver for this step) | –                                                                                                       | `{code:string; expiresAt:ISODateTime; redeemableByUserId:UUID; documentType:string; documentId:UUID}`                                                                                                                                                                                                                                                                                | B-15           |

**One-time approval codes (B-15, owner decision 2026-08-22).** `POST
`viewerCanDecide`answers "may THIS caller decide the step that is currently
waiting". It is resolved server-side with`resolveEligibleRoles`+`isRoleAuthorized`— the same pair`transition()`uses, so it carries the §5
rank override — and it exists because a permission is not the same question: a
document type's approve permission is an any-of over EVERY step's role, so a
Supervisor holding the outlet key passed a naive check on the warehouse's step
and was offered a decision the server then refused. It is OPTIONAL because only
this resource knows the caller; the document resources that embed an approval
render a read-only timeline and do not answer it, so a consumer gating an
action must test`=== true` and fail closed.

`actedByName` is the approver's NAME for display. `actedBy` remains their user
id — the audit fact — and must never be rendered: it reached a screen once as
"disetujui oleh 640218f4-cdbd-4d65-80ae-8b1c31ececc0".

/api/approvals/:documentType/:documentId/code`is the AUTHORIZATION act for a
document whose decision must be completed by someone else — today`void_refund`. The approver calls it from their own session; the six-digit code
it returns is single-use, expires in 5 minutes, is bound to that one document,
and may be redeemed only by the document's requester. Redeeming it (e.g. `POST
/api/pos/void-refunds/:id/approve`with`{code}`) records the decision against
the APPROVER, not the redeemer.

Two gates apply: the permission key, and a per-step eligibility check against
the §5 state machine — holding `approval.code.issue` is not sufficient. Wrong
codes lock the CALLER (5 attempts / 15 minutes, with backoff at 3 and 4), never
the approver; a hard lock is cleared via `POST /api/auth/lockouts/:userId/clear`
by someone of strictly higher `ROLE_RANK`.

This replaced `POST /api/auth/pin/verify`, which was removed — see §4.1.
...
| POST   | `/api/auth/lockouts/:userId/clear`                       | `auth.lockout.clear` (+ must outrank the locked user)       | –                                                        | `{ok:true; cleared:boolean}`                                                                                                                | B-15              |
| GET    | `/api/auth/lockouts/me`                                  | (any; self only)                                            | –                                                        | `{locked:boolean; hardLocked:boolean; lockedUntil:ISODateTime\|null}`                                                                       | B-15              |
| POST   | `/api/auth/offline-credential/refresh`                   | `auth.offline_credential.mint`                              | `{deviceId:UUID}`                                        | `OfflineCredentialRes`                                                                                                                      | D-17              |
| POST   | `/api/auth/offline-credential/:credentialId/unlock-code` | `auth.lockout.clear` (+ must outrank the credential holder) | `{challenge:string}` (6 digits, generated by the tablet) | `{code:string; credentialId:UUID}` (8 chars, read to the outlet by phone)                                                                   | B-17              |
...
> **Offline unlock (B-17, owner Q7).** A tablet locks a cached credential after
> five wrong PINs. Online that is fixed by re-issuing the credential; during an
> outage there was previously no way back at all. The device now backs off first
> (30 s at the 3rd wrong PIN, 2 min at the 4th — the same ladder as B-15's
> server-side limiter) and only then locks terminally. To recover from a terminal
> lock the tablet shows a 6-digit CHALLENGE; head office calls
> `/unlock-code` with it and reads back an 8-character CODE; the device verifies
> that code against the binding secret it already holds, with no connectivity.
>
> The derivation lives once in `@mimi/shared` (`unlockCodeMessage` /
> `encodeUnlockCode`) and both tiers assert against the same known-answer
> fixture. The server reads the credential through migration 206's
> `SECURITY DEFINER` lookup, which cannot return `pin_verifier` — so this path
> never hands a central role an argon2id hash of anyone's PIN.

> **`POST /api/auth/pin/verify` was REMOVED (B-15, owner decision 2026-08-22).**
> It accepted an arbitrary `userId` and answered "is this PIN correct?" for any
> user, unthrottled — an oracle against the same PIN that backs offline
> authorization on tablets. The flow it served (FR-POS-03: a supervisor
> authorises an action at the till) now runs on ONE-TIME APPROVAL CODES:
> `POST /api/approvals/:documentType/:documentId/code` (§4.0). Do not
> reintroduce a "verify this user's PIN" endpoint in any form.
>
> `POST /api/auth/pin` (set) remains: a PIN is still the credential for the
> OFFLINE path (`offline_credentials.pin_verifier`), where no server exists to
> mint a code.
...
| GET    | `/api/delivery/recap/daily`              | `report.logistics.read`  | `?date=`                                                                                                                                                                                                                                                                                                                                                        | `{date:ISODate; sjCount:number; dropCount:number; byCity:{city:string; outlets:number; sjCount:number; dropCount:number; frozenSjCount:number; drySjCount:number; items:{itemId:UUID; itemName:string; qty:Qty}[]; byOutlet:{locationId:UUID; locationName:string; sjCount:number; dropCount:number; frozenSjCount:number; drySjCount:number; items:{itemId:UUID; itemName:string; qty:Qty}[]}[]}[]; frozenSjCount:number; drySjCount:number}` | FR-LOG-04/08             |
...
| POST   | `/api/pos/void-refunds/:id/approve`   | `pos.void.request` (B-15 — the till redeems; approval authority comes from the code) | `{code:string}` (6 digits, one-time, issued via `/api/approvals/void_refund/:id/code`) online; offline path = `void_refunds.approved_offline` event (§7.3)                                                                                                                                                                                                                                                                               | `{id; status:'approved'; offlineAuthorized:boolean}` — reverses payments/usage, journal `SALE_VOID_REVERSAL`                                                                                                                                                                                                                                                        | FR-POS-03, APR-02, D-17          |
...
| POST   | `/api/accounting/payments/:id/pay`       | `payment.pay`                | `{paidVia:'cash'\|'bank_transfer'\|'qris'; paidAt?:ISODateTime}` — amounts ≥ `approval.threshold.payment` need the Owner approval step first (§5.8) — `ERR_APPROVAL_REQUIRED` while that step is pending, and the step is RAISED at `verify` (so the Owner is notified then) and DECIDED here, action `'pay'`; posts the §6 payment journal for the ref type | `PaymentVerification` (paid)                                                                                                                                                                                                                                                                                                                                                                                                                               | FR-ACCT-03/04, APR-08    |
...
| **X8**  | **`SUPPLIER_PAYMENT`**                                                             | **PV `paid`, `ref_type='purchase_order'`**                                                                                 | **2000 Hutang Supplier**                                                           | **1020 Bank / 1000 Kas (per `paid_via`; QRIS → 1020)**                                                 | `pv.amount`                    | **AMENDMENT (2026-09-09): §6 named NO rule that ever debited 2000 except JGUD-04 (retur). JGUD-01 credits it at PO receipt, so paying a supplier never cleared the payable and never recorded the cash outflow — `Hutang Supplier` grew monotonically. Measured on the dev DB when found: `SUM(debit) = 0.00` against `SUM(credit) = 30185175.00`.** |
| **X9**  | **`MAINTENANCE_PAYMENT`**                                                          | **PV `paid`, `ref_type='maintenance_job'`**                                                                                | **6200 Beban Maintenance**                                                         | **1020 / 1000 (per `paid_via`)**                                                                       | `pv.amount`                    | **AMENDMENT (2026-09-09): `asset/jobs.service.ts` posts no journal at job completion (it only opens the PV, FR-ACCT-04), so payment is the sole recognition point. 6200 is seeded in §6.1 and was referenced by NO posting rule — maintenance cost never reached the P&L in any form.**                                                              |
| **X10** | **`EMPLOYEE_COMPENSATION_PAYMENT`**                                                | **PV `paid`, `ref_type='incentive'` or `'thr'`**                                                                           | **6000 Beban Gaji**                                                                | **1020 / 1000 (per `paid_via`)**                                                                       | `pv.amount`                    | **AMENDMENT (2026-09-09): insentif/THR are paid OUTSIDE a payroll run, so no X1 accrual against 2100 Hutang Gaji exists to settle — debiting the liability would invent one. Neither ref_type posted anything before this.**                                                                                                                         |

Petty-cash float top-up (`Dr 1010 / Cr 1020`) and loan disbursement (`Dr 1210 / Cr 1020`) post from their PV `paid` events under X-family rules — dispatched on `ref_type='petty_cash'|'employee_loan'`, the CHECK-constrained column (migration 259 added `employee_loan`). An earlier implementation keyed both off a `#kind:` marker written into `payment_verifications.notes`; no production code ever wrote that marker, so both rules sat unreachable from migration 093 until 2026-09-09. The `petty_cash` case is the float REPLENISHMENT: JOUT-07/08 already posted the expense at verify, crediting 1010 and drawing the float down, so the PV `paid` is what puts it back.

**Every one of the ten `payment_verifications.ref_type` values now has a posting rule** (`purchase_order` X8, `payroll_run` X2, `petty_cash` top-up, `maintenance_job` X9, `sale_payment` X3/X4, `online_order` X5, `incentive`/`thr` X10, `employee_loan` disbursement, `other` JOUT-09). Before 2026-09-09 five of them fell into a `default:` in `PaymentVerificationsService.publishPaymentJournal` that returned silently. JOUT-09's "+ outlet location" wording had also been implemented as a hard guard, so a voucher with no `location_id` posted nothing at all; a location-less expense is a central/HQ expense, not a non-expense, and now posts to 6100 with `journal_entries.location_id` left NULL. A dedicated central-expense account (there is no 61xx "Beban Operasional Pusat" in §6.1) is the open follow-up for that naming.
